/*
  ...
  6. Usa somente três níveis de velocidade nos motores: 0, 210 e 255.
...
// Ela só pode assumir 0, 210 ou 255.
...
// Exemplo: se o comando frente() faz o lado esquerdo ir para trás,
// coloque INVERTER_MOTOR_E como true.
#define INVERTER_MOTOR_E false
#define INVERTER_MOTOR_D false

int normalizarVelocidade(int velocidade) {
  // Força qualquer velocidade a virar somente 0, 210 ou 255.
...
void motorRaw(int pinoFrente, int pinoRe, int pinoPWM, int velocidadeAssinada, bool inverter) {
  // velocidadeAssinada:
  //   positiva = frente
  //   negativa = ré
  //   zero     = parado
...
void motorEsquerdo(int velocidadeAssinada) {
  motorRaw(MOTOR_E_FRENTE, MOTOR_E_RE, MOTOR_E_PWM, velocidadeAssinada, INVERTER_MOTOR_E);
}

void motorDireito(int velocidadeAssinada) {
  motorRaw(MOTOR_D_FRENTE, MOTOR_D_RE, MOTOR_D_PWM, velocidadeAssinada, INVERTER_MOTOR_D);
}
...
void aplicarJoystick() {
  // Joystick corrigido para manter somente as velocidades 0, 210 e 255,
  // mas ainda permitir curva de verdade.
  //
  // velocidadeManual = 210:
  //   frente        -> 210 / 210
  //   curva direita -> 210 / 0
  //   curva esquerda-> 0   / 210
  //
  // velocidadeManual = 255:
  //   frente        -> 255 / 255
  //   curva direita -> 255 / 210
  //   curva esquerda-> 210 / 255
  ...
  // Com velocidade 210, a roda lenta para.
  // Com velocidade 255, a roda lenta fica em 210.
  int lenta = (base == VEL_MAXIMA) ? VEL_MEDIA : VEL_PARADO;

  // Giro no próprio eixo: joystick para direita/esquerda sem frente/ré.
...
int velocidadePorErroRSSI(int erroAbsoluto) {
  // Converte o tamanho do erro em 0, 210 ou 255.
  // Erro pequeno: para.
