...
// =====================================================
// FUNÇÕES DE VELOCIDADE, MOTORES E ACELERAÇÃO
// =====================================================

int normalizarVelocidade(int velocidade) {
  // Força qualquer velocidade de comando a virar somente 0, 210 ou 255.
...
int normalizarVelocidadeAssinada(int velocidadeAssinada) {
  if (velocidadeAssinada == 0) return 0;

  int sinal = velocidadeAssinada > 0 ? 1 : -1;
  int vel = normalizarVelocidade(abs(velocidadeAssinada));
  return sinal * vel;
}

void motorRawDireto(int pinoFrente, int pinoRe, int pinoPWM, int velocidadeAssinada, bool inverter) {
  // velocidadeAssinada:
  //   positiva = frente
  //   negativa = ré
  //   zero     = parado
  // Esta função escreve o PWM real, incluindo valores intermediários da aceleração.

  if (inverter) {
    velocidadeAssinada = -velocidadeAssinada;
  }

  int velocidade = constrain(abs(velocidadeAssinada), 0, VEL_MAXIMA);
...
int aproximarPWM(int atual, int alvo) {
  ...
  // Se vai inverter direção, primeiro freia até zero.
  if (atual != 0 && alvo != 0 && ((atual > 0 && alvo < 0) || (atual < 0 && alvo > 0))) {
    alvo = 0;
  }

  int passo = abs(alvo) > abs(atual) ? ACEL_PASSO_SUBIDA : ACEL_PASSO_DESCIDA;
...
void aplicarPWMAtual() {
  motorRawDireto(MOTOR_E_FRENTE, MOTOR_E_RE, MOTOR_E_PWM, atualVelEsq, INVERTER_MOTOR_E);
  motorRawDireto(MOTOR_D_FRENTE, MOTOR_D_RE, MOTOR_D_PWM, atualVelDir, INVERTER_MOTOR_D);
}
...
void moverMotores(int velEsq, int velDir) {
  alvoVelEsq = normalizarVelocidadeAssinada(velEsq);
  alvoVelDir = normalizarVelocidadeAssinada(velDir);
}
...
void aplicarJoystick() {
  ...
  if (y > 0) {
    if (x > limiteCurva) {
      moverMotores(base, lenta);
      ultimaAcao = "Curva direita";
    } else if (x < -limiteCurva) {
      moverMotores(lenta, base);
...
void seguirDispositivo() {
  ...
  if (rssiSuavizado > rssiMuitoPertoDin) {
    ...
    // Se o celular estiver muito perto, o carrinho fica parado.
    parar();
    inicioBusca = 0;
    faseCorrecao = 0;
    emCorrecao = false;
...
void setup() {
  ...
  Serial.println("Carrinho ESP32 iniciado com abas, presets RSSI e aceleração progressiva.");
